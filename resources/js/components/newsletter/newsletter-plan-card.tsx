...
export function NewsletterPlanCard({ plan }: NewsletterPlanCardProps) {
    ...
    function handleSubmit(e: FormEvent) {
        ...
        router.post(
            "/newsletter",
            { email, newsletter: plan.id }, // was: { email, plan: plan.id }
            {
                preserveScroll: true,
    ...
    return (
        <div className="flex flex-col rounded-sm border border-border p-6">
            <Badge
                variant="secondary"
                className="w-fit text-[10px] font-semibold tracking-widest uppercase"
            >
                {plan.badge}
            </Badge>
            ...
            <Link
                href={plan.previewHref}
                className="mt-3 text-center text-sm text-muted-foreground underline underline-offset-2 hover:text-foreground"
            >
